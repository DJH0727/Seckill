...
public class StockSyncTask {
    ...
    public void syncStockToDB() {
        Set<String> dirtyProductIds = redisTemplate.opsForSet().members( RedisKey.STOCK_DIRTY_SET);
        if (dirtyProductIds == null || dirtyProductIds.isEmpty()) {
            return;
        }

        for (String productIdStr : dirtyProductIds) {
            String redisKey = RedisKey.STOCK_KEY_PREFIX + productIdStr;
            ...
            redisTemplate.opsForSet().remove(RedisKey.STOCK_DIRTY_SET, productIdStr);
        }
